...
#define	NOKIA5110_RST_LOW()		(NOKIA5110_PORT->ODR &=(uint8_t)(~NOKIA5110_RST))
#define	NOKIA5110_CE_HIGH()		(NOKIA5110_PORT->ODR |= NOKIA5110_CE)
#define	NOKIA5110_CE_LOW()		(NOKIA5110_PORT->ODR &=(uint8_t)(~NOKIA5110_CE))
#define	NOKIA5110_DC_HIGH()		(NOKIA5110_PORT->ODR |= NOKIA5110_DC)
#define	NOKIA5110_DC_LOW()		(NOKIA5110_PORT->ODR &=(uint8_t)(~NOKIA5110_DC))
#define	NOKIA5110_DIN_HIGH()	(NOKIA5110_PORT->ODR |= NOKIA5110_DIN)
#define	NOKIA5110_DIN_LOW()		(NOKIA5110_PORT->ODR &=(uint8_t)(~NOKIA5110_DIN))
#define	NOKIA5110_CLK_HIGH()	(NOKIA5110_PORT->ODR |= NOKIA5110_CLK)
#define	NOKIA5110_CLK_LOW()		(NOKIA5110_PORT->ODR &=(uint8_t)(~NOKIA5110_CLK))
...
static uint8_t print_oneChineseWord(uint8_t Row,uint8_t Column,uint8_t *pString)
{
	...
	while(*FontIndex[i]!=0)
	{
		if((*pString==*FontIndex[i])&&(*(pString+1)==*(FontIndex[i]+1)))																										//找到字库数据
		{
			...
			//显示区清零同时显示			
			shang_begin=(uint8_t)(Row/8);
			yushu_begin=(uint8_t)(Row%8);
			shang_end=(uint8_t)((Row+12-1)/8);
			yushu_end=(uint8_t)((Row+12-1)%8);		
...
{
uint8_t char_string[2]={0,'\0'};
uint8_t m8;
	m8=(uint8_t)(num>>4);
	if(m8>9){
		char_string[0]=(uint8_t)('A'+m8-10);		
	}else{
		char_string[0]=(uint8_t)('0'+m8);		
	}
	print_oneChar(Row,Column,&char_string[0]);
	m8=(uint8_t)(num&0x0f);
	if(m8>9){
		char_string[0]=(uint8_t)('A'+m8-10);
	}else{
	...
	print_oneChar(Row,(uint8_t)(Column+6),&char_string[0]);	
}
/*
num: (0-9)
*/
...
{
	uint8_t char_string[2];
	num&=0x0f;	//实际应为num%=10,此处为加快效率采用了不严格限定
	char_string[0]=(uint8_t)('0'+num);
	char_string[1]='\0';
	print_oneChar(Row,Column,&char_string[0]);
}
